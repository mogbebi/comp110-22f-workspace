    ...
    while still_playing:
        ...
        if user_choice == choice_one:
            if first_time:
                ...
                print(f"Speaking of which... what do you want to test first?")
            test_library()
...
def sink_test(result: float):
    ...
    if points >= 1000:
        print(f"At this rate, you're gonna be rolling in dough after four years of undergrad!")
    ...
    while i < len(dorm_names_years):
        if dorm_names_years[i] == dorm_input:
            ...
            if dorm_names_years[i + 1] == 1795:
                print(f"WOW, The Big Kahuna! This is the oldest dorm on campus, meaning it has the highest lead levels of them all. Who needs a scholarship when you've got all this money!?")
                global points
                return 10000
            elif 1900 < dorm_names_years[i + 1] < 1930:
                print(f"That's one the oldest dorms on campus, so it's likely to have the most lead! Lucky you!")
                return randint(4001, 6000)
            elif 1931 < dorm_names_years[i + 1] < 1960:
                print(f"That dorm's pretty old, but not the oldest there is. You're only getting moderate amount of lead!")
                ...
                print(f"Dang, you're living in one of the modern dorms. No lead (or money) for you; better luck next year!")
                return 0
        i += 2
